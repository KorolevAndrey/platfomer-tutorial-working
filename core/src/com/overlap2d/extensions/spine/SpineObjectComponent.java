...
public class SpineObjectComponent implements Component {
    ...
    public void computeBoundBox(DimensionsComponent dimensionsComponent) {
        ...
        for (int i = 0, n = skeleton.getSlots().size; i < n; i++) {
            ...
            //float[] Rvertices = new float[20];
            //float[] Mvertices = new float[120];
            if ((attachment instanceof RegionAttachment)) {
                RegionAttachment region = (RegionAttachment) attachment;
                //Rvertices = region.getUVs();
                //region.computeWorldVertices(slot.getBone(), Rvertices, 0,0);
            ...
            if ((attachment instanceof MeshAttachment)) {
                ...
                //Mvertices = region.getWorldVertices();
                region.updateWorldVertices(slot, false);
                //region.computeWorldVertices(slot,0,0, Mvertices, 0,0);
                vertices = region.getWorldVertices();
                //vertices = Mvertices;
            }
